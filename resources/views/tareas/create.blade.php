...
<div class="container">
    ...
    <form action="{{ route('tareas.store') }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control @error('titulo') is-invalid @enderror" id="titulo" name="titulo" required value="{{ old('titulo') }}">
            @error('titulo')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control datepicker @error('fecha_vencimiento') is-invalid @enderror" id="fecha_vencimiento" name="fecha_vencimiento" required value="{{ old('fecha_vencimiento') }}">
        ...
        <div class="form-group">
            ...
            <select class="form-control select2 @error('asignado_id') is-invalid @enderror" id="asignado_id" name="asignado_id">
                ...
                @foreach($usuarios as $usuario)
                    <option value="{{ $usuario->id }}" {{ old('asignado_id') == $usuario->id ? 'selected' : '' }}>{{ $usuario->name }}</option>
                @endforeach
            </select>
            @error('asignado_id')
                <div class="invalid-feedback">{{ $message }}</div>
